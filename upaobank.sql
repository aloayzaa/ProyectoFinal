...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 06-07-2016 a las 17:19:38
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_deposito`(	p_cuenta char(8), 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `usp_registrar_cliente`(ap_paterno varchar(25), 
...
BEGIN
    ...
    set codcliente = (select (LPAD(chr_cliecodigo+1,5,'0')) as codcliente from cliente order by chr_cliecodigo desc limit 1);
    	set cont := cont + 1;
	insert into cliente(chr_cliecodigo, vch_cliepaterno,vch_cliematerno, 
    vch_clienombre, chr_cliedni, vch_clieciudad, vch_cliedireccion, vch_clietelefono,vch_clieemail)
	values(codcliente,ap_paterno,ap_materno,nombres,dni,'Trujillo','Ninguna',null,correo);
    		
	-- Confirma Transacción
	commit;

	-- Retorna el estado
	select 1 as state, 'Registro realizado' as message;
...
('00001', 'MENDOZA', 'PUERTA', 'HENRY', '06914897', 'TRUJILLO', 'LOS OLIVOS', '9666-4457', 'hmendo81@gmail.com'),
...
('00100001', '01', '001', '0004', '00005', '6900.00', '2008-01-06', 'ACTIVO', 7, '123456'),
...
('0003', 'Reyes', 'Ortiz', 'Claudia', 'Lima', 'Av. Aviaci?n 3456 - San Borja', 'creyes', 'linda'),
...
('002', 'N?mero de Operaciones Sin Costo', '15', 'ACTIVO');
...
ALTER TABLE `asignado`
  ADD PRIMARY KEY (`chr_asigcodigo`),
  ADD KEY `idx_asignado01` (`chr_emplcodigo`),
  ADD KEY `idx_asignado02` (`chr_sucucodigo`);
...
ALTER TABLE `cargomantenimiento`
  ...
  ADD KEY `idx_cargomantenimiento01` (`chr_monecodigo`);
...
ALTER TABLE `costomovimiento`
  ADD PRIMARY KEY (`chr_monecodigo`),
  ADD KEY `idx_costomovimiento` (`chr_monecodigo`);

--
-- Indices de la tabla `cuenta`
--
ALTER TABLE `cuenta`
  ADD PRIMARY KEY (`chr_cuencodigo`),
  ADD KEY `idx_cuenta01` (`chr_cliecodigo`),
  ADD KEY `idx_cuenta02` (`chr_emplcreacuenta`),
  ADD KEY `idx_cuenta03` (`chr_sucucodigo`),
  ADD KEY `idx_cuenta04` (`chr_monecodigo`);

--
-- Indices de la tabla `empleado`
--
ALTER TABLE `empleado`
  ADD PRIMARY KEY (`chr_emplcodigo`),
  ADD UNIQUE KEY `U_Empleado_vch_emplusuario` (`vch_emplusuario`);
...
ALTER TABLE `interesmensual`
  ADD PRIMARY KEY (`chr_monecodigo`),
  ADD KEY `idx_interesmensual01` (`chr_monecodigo`);
...
ALTER TABLE `movimiento`
  ADD PRIMARY KEY (`chr_cuencodigo`,`int_movinumero`),
  ADD KEY `idx_movimiento01` (`chr_tipocodigo`),
  ADD KEY `idx_movimiento02` (`chr_emplcodigo`),
  ADD KEY `idx_movimiento03` (`chr_cuencodigo`);
